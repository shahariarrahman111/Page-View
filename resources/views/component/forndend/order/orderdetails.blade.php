<main>
    <div class="main-content">
        <div class="container-fluid">
            <div class="userDashboard d-flex">
                <div class="sidebarMenu">
                    <div class="userPanel mt-3">
                        <div class="user-profile  d-flex align-items-center flex-column justify-content-center gap-2">
                            <div class="profile-img">
                                <img
                                    src="https://images.unsplash.com/photo-1633332755192-727a05c4013d?q=80&amp;w=2080&amp;auto=format&amp;fit=crop&amp;ixlib=rb-4.0.3&amp;ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"
                                    alt="">
                            </div>
                            <h6>Md Abdul Mannan</h6>
                        </div>
                        <ul class="navbar-nav d-flex  ">
                            <li class="nav-item">
                                <a href="userdashboard.html" class="nav-link " aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="dashboard" fill-rule="evenodd" clip-rule="evenodd"
                                              d="M6.25 3.5C3.5 3.5 3.5 3.708 3.5 6.25V6.275C3.5 7.382 3.5 8.182 3.771 8.52C4.036 8.848 4.823 9 6.25 9C7.677 9 8.464 8.847 8.729 8.519C9 8.182 9 7.382 9 6.274C9 3.708 9 3.5 6.25 3.5ZM6.25 10.5C4.564 10.5 3.299 10.323 2.604 9.46C2 8.711 2 7.689 2 6.275L2.75 6.25H2C2 3.38 2.181 2 6.25 2C10.319 2 10.5 3.38 10.5 6.25C10.5 7.688 10.5 8.711 9.896 9.46C9.201 10.323 7.936 10.5 6.25 10.5Z"
                                              fill="#475569"></path>
                                        <path class="dashboard" fill-rule="evenodd" clip-rule="evenodd"
                                              d="M17.25 3.5C14.5 3.5 14.5 3.708 14.5 6.25V6.275C14.5 7.382 14.5 8.182 14.771 8.52C15.036 8.848 15.823 9 17.25 9C18.677 9 19.464 8.847 19.729 8.519C20 8.182 20 7.382 20 6.274C20 3.708 20 3.5 17.25 3.5ZM17.25 10.5C15.564 10.5 14.299 10.323 13.604 9.46C13 8.711 13 7.689 13 6.275L13.75 6.25H13C13 3.38 13.181 2 17.25 2C21.319 2 21.5 3.38 21.5 6.25C21.5 7.688 21.5 8.711 20.896 9.46C20.201 10.323 18.936 10.5 17.25 10.5Z"
                                              fill="#475569"></path>
                                        <path class="dashboard" fill-rule="evenodd" clip-rule="evenodd"
                                              d="M6.25 14.5C3.5 14.5 3.5 14.708 3.5 17.25V17.275C3.5 18.382 3.5 19.182 3.771 19.52C4.036 19.848 4.823 20 6.25 20C7.677 20 8.464 19.847 8.729 19.519C9 19.182 9 18.382 9 17.274C9 14.708 9 14.5 6.25 14.5ZM6.25 21.5C4.564 21.5 3.299 21.323 2.604 20.46C2 19.711 2 18.689 2 17.275L2.75 17.25H2C2 14.38 2.181 13 6.25 13C10.319 13 10.5 14.38 10.5 17.25C10.5 18.688 10.5 19.711 9.896 20.46C9.201 21.323 7.936 21.5 6.25 21.5Z"
                                              fill="#475569"></path>
                                        <path class="dashboard" fill-rule="evenodd" clip-rule="evenodd"
                                              d="M17.25 14.5C14.5 14.5 14.5 14.708 14.5 17.25V17.275C14.5 18.382 14.5 19.182 14.771 19.52C15.036 19.848 15.823 20 17.25 20C18.677 20 19.464 19.847 19.729 19.519C20 19.182 20 18.382 20 17.274C20 14.708 20 14.5 17.25 14.5ZM17.25 21.5C15.564 21.5 14.299 21.323 13.604 20.46C13 19.711 13 18.689 13 17.275L13.75 17.25H13C13 14.38 13.181 13 17.25 13C21.319 13 21.5 14.38 21.5 17.25C21.5 18.688 21.5 19.711 20.896 20.46C20.201 21.323 18.936 21.5 17.25 21.5Z"
                                              fill="#475569"></path>
                                    </svg>
                                    Dashboard</a>
                            </li>
                            <li class="nav-item">
                                <a href="order.html" class="nav-link active" aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="order"
                                              d="M12 22C11.1818 22 10.4002 21.6698 8.83693 21.0095C4.94564 19.3657 3 18.5438 3 17.1613C3 16.7742 3 10.0645 3 7M12 22C12.8182 22 13.5998 21.6698 15.1631 21.0095C19.0544 19.3657 21 18.5438 21 17.1613V7M12 22V11.3548"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"
                                              stroke-linejoin="round"></path>
                                        <path class="order"
                                              d="M8.32592 9.69138L5.40472 8.27785C3.80157 7.5021 3 7.11423 3 6.5C3 5.88577 3.80157 5.4979 5.40472 4.72215L8.32592 3.30862C10.1288 2.43621 11.0303 2 12 2C12.9697 2 13.8712 2.4362 15.6741 3.30862L18.5953 4.72215C20.1984 5.4979 21 5.88577 21 6.5C21 7.11423 20.1984 7.5021 18.5953 8.27785L15.6741 9.69138C13.8712 10.5638 12.9697 11 12 11C11.0303 11 10.1288 10.5638 8.32592 9.69138Z"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"
                                              stroke-linejoin="round"></path>
                                        <path class="order" d="M6 12L8 13" stroke="#475569" stroke-width="1.5"
                                              stroke-linecap="round" stroke-linejoin="round"></path>
                                        <path class="order" d="M17 4L7 9" stroke="#475569" stroke-width="1.5"
                                              stroke-linecap="round" stroke-linejoin="round"></path>
                                    </svg>
                                    My Order</a>

                            </li>
                            <li class="nav-item">
                                <a href="profile.html" class="nav-link" aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="profile"
                                              d="M6.57757 15.4816C5.1628 16.324 1.45336 18.0441 3.71266 20.1966C4.81631 21.248 6.04549 22 7.59087 22H16.4091C17.9545 22 19.1837 21.248 20.2873 20.1966C22.5466 18.0441 18.8372 16.324 17.4224 15.4816C14.1048 13.5061 9.89519 13.5061 6.57757 15.4816Z"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"
                                              stroke-linejoin="round"></path>
                                        <path class="profile"
                                              d="M16.5 6.5C16.5 8.98528 14.4853 11 12 11C9.51472 11 7.5 8.98528 7.5 6.5C7.5 4.01472 9.51472 2 12 2C14.4853 2 16.5 4.01472 16.5 6.5Z"
                                              stroke="#475569" stroke-width="1.5"></path>
                                    </svg>
                                    Profile</a>
                            </li>
                            <li class="nav-item">
                                <a href="address.html" class="nav-link" aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="address"
                                              d="M13.6177 21.367C13.1841 21.773 12.6044 22 12.0011 22C11.3978 22 10.8182 21.773 10.3845 21.367C6.41302 17.626 1.09076 13.4469 3.68627 7.37966C5.08963 4.09916 8.45834 2 12.0011 2C15.5439 2 18.9126 4.09916 20.316 7.37966C22.9082 13.4393 17.599 17.6389 13.6177 21.367Z"
                                              stroke="#475569" stroke-width="1.5"></path>
                                        <path class="address"
                                              d="M15.5 11C15.5 12.933 13.933 14.5 12 14.5C10.067 14.5 8.5 12.933 8.5 11C8.5 9.067 10.067 7.5 12 7.5C13.933 7.5 15.5 9.067 15.5 11Z"
                                              stroke="#475569" stroke-width="1.5"></path>
                                    </svg>
                                    Shipping address</a>

                            </li>
                            <li class="nav-item">
                                <a href="setting.html" class="nav-link" aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="setting"
                                              d="M21.3175 7.14139L20.8239 6.28479C20.4506 5.63696 20.264 5.31305 19.9464 5.18388C19.6288 5.05472 19.2696 5.15664 18.5513 5.36048L17.3311 5.70418C16.8725 5.80994 16.3913 5.74994 15.9726 5.53479L15.6357 5.34042C15.2766 5.11043 15.0004 4.77133 14.8475 4.37274L14.5136 3.37536C14.294 2.71534 14.1842 2.38533 13.9228 2.19657C13.6615 2.00781 13.3143 2.00781 12.6199 2.00781H11.5051C10.8108 2.00781 10.4636 2.00781 10.2022 2.19657C9.94085 2.38533 9.83106 2.71534 9.61149 3.37536L9.27753 4.37274C9.12465 4.77133 8.84845 5.11043 8.48937 5.34042L8.15249 5.53479C7.73374 5.74994 7.25259 5.80994 6.79398 5.70418L5.57375 5.36048C4.85541 5.15664 4.49625 5.05472 4.17867 5.18388C3.86109 5.31305 3.67445 5.63696 3.30115 6.28479L2.80757 7.14139C2.45766 7.74864 2.2827 8.05227 2.31666 8.37549C2.35061 8.69871 2.58483 8.95918 3.05326 9.48012L4.0843 10.6328C4.3363 10.9518 4.51521 11.5078 4.51521 12.0077C4.51521 12.5078 4.33636 13.0636 4.08433 13.3827L3.05326 14.5354C2.58483 15.0564 2.35062 15.3168 2.31666 15.6401C2.2827 15.9633 2.45766 16.2669 2.80757 16.8741L3.30114 17.7307C3.67443 18.3785 3.86109 18.7025 4.17867 18.8316C4.49625 18.9608 4.85542 18.8589 5.57377 18.655L6.79394 18.3113C7.25263 18.2055 7.73387 18.2656 8.15267 18.4808L8.4895 18.6752C8.84851 18.9052 9.12464 19.2442 9.2775 19.6428L9.61149 20.6403C9.83106 21.3003 9.94085 21.6303 10.2022 21.8191C10.4636 22.0078 10.8108 22.0078 11.5051 22.0078H12.6199C13.3143 22.0078 13.6615 22.0078 13.9228 21.8191C14.1842 21.6303 14.294 21.3003 14.5136 20.6403L14.8476 19.6428C15.0004 19.2442 15.2765 18.9052 15.6356 18.6752L15.9724 18.4808C16.3912 18.2656 16.8724 18.2055 17.3311 18.3113L18.5513 18.655C19.2696 18.8589 19.6288 18.9608 19.9464 18.8316C20.264 18.7025 20.4506 18.3785 20.8239 17.7307L21.3175 16.8741C21.6674 16.2669 21.8423 15.9633 21.8084 15.6401C21.7744 15.3168 21.5402 15.0564 21.0718 14.5354L20.0407 13.3827C19.7887 13.0636 19.6098 12.5078 19.6098 12.0077C19.6098 11.5078 19.7888 10.9518 20.0407 10.6328L21.0718 9.48012C21.5402 8.95918 21.7744 8.69871 21.8084 8.37549C21.8423 8.05227 21.6674 7.74864 21.3175 7.14139Z"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"></path>
                                        <path class="setting"
                                              d="M15.5195 12C15.5195 13.933 13.9525 15.5 12.0195 15.5C10.0865 15.5 8.51953 13.933 8.51953 12C8.51953 10.067 10.0865 8.5 12.0195 8.5C13.9525 8.5 15.5195 10.067 15.5195 12Z"
                                              stroke="#475569" stroke-width="1.5"></path>
                                    </svg>
                                    Setting</a>

                            </li>
                            <li class="nav-item">
                                <a href="index.html" class="nav-link" aria-current="page">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path class="logout"
                                              d="M15 17.625C14.9264 19.4769 13.3831 21.0494 11.3156 20.9988C10.8346 20.987 10.2401 20.8194 9.05112 20.484C6.18961 19.6768 3.70555 18.3203 3.10956 15.2815C3 14.723 3 14.0944 3 12.8373V11.1627C3 9.90561 3 9.27705 3.10956 8.71846C3.70555 5.67965 6.18961 4.32316 9.05112 3.51603C10.2401 3.18064 10.8346 3.01295 11.3156 3.00119C13.3831 2.95061 14.9264 4.52307 15 6.37501"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"></path>
                                        <path class="logout"
                                              d="M21 12H10M21 12C21 11.2998 19.0057 9.99153 18.5 9.5M21 12C21 12.7002 19.0057 14.0085 18.5 14.5"
                                              stroke="#475569" stroke-width="1.5" stroke-linecap="round"
                                              stroke-linejoin="round"></path>
                                    </svg>

                                    Logout</a>

                            </li>

                        </ul>
                    </div>
                </div>
                <div class="sidebarContent">
                    <div class="myOrder">
                        <div class=" d-flex justify-content-between mb-3">
                            <div>
                                <h6>Order ID : <span class="text-primary"> #452140</span></h6>
                                <p class="mt-1">Placed on : 29 Mar 2023 11:20 PM</p>
                            </div>


                        </div>

                        <div class="orderDetails">
                            <div class="order-status-bar d-flex align-items-center justify-content-center flex-column ">
                                <div class="estimate-time d-flex align-items-center flex-column">
                                    <p class="font-16 fw-500 text-black mb-2">Status Bar</p>
                                    <p class="text-center">Estimated Delivery By Wed 31 Mar - Sat 02 Apr</p>
                                </div>
                                <div class="progress-track w-100">
                                    <ul class="progressbar d-flex w-100">
                                        <li class="is_complete">
                                            <p>Order Placed</p>
                                        </li>
                                        <li class="is_complete">
                                            <p>Approved</p>
                                        </li>
                                        <li class="is_complete">
                                            <p>Processing </p>
                                        </li>

                                        <li>
                                            <p>Packed</p>
                                        </li>
                                        <li>
                                            <p>Shipped </p>
                                        </li>
                                        <li>
                                            <p>Delivered</p>
                                        </li>

                                    </ul>
                                </div>
                            </div>
                            <div class="pageTitle d-flex justify-content-between mt-5 ">
                                <h5>Item from order</h5>
                            </div>
                            <div class="tableContainer mt-3">
                                <div class="cartItem row align-items-center">
                                    <div class="col">
                                        <div class=" d-flex flex-column">
                                            <p class="font-16 fw-500">Product Name</p>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="itemDetails d-flex align-items-center justify-content-between">
                                            <div class=" d-flex flex-column">
                                                <p class="font-16 fw-500">Price</p>
                                            </div>
                                            <div class=" d-flex flex-column">
                                                <p class="font-16 fw-500">Quantity</p>
                                            </div>
                                            <div class="itemPrice d-flex flex-column">
                                                <p class="font-16 fw-500">Sub-total</p>
                                            </div>


                                        </div>
                                    </div>
                                </div>

                                <div class="cartItem row">
                                    <div class="col">
                                        <div class="itemName d-flex gap-2 ">
                                            <div class="img">
                                                <img src="assets/images/product4.png" alt="">
                                            </div>
                                            <div class="name ">
                                                <p class="font-16 fw-500">Dark blue - Block Dress Dark blue - Block Dress </p>
                                                <p class="font-16 mt-1">Color : Red </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="itemDetails d-flex align-items-center justify-content-between">
                                            <div class=" d-flex ">
                                                <p class="font-16 fw-500 d-none">Price</p>
                                                <p class="font-16 fw-500">100</p>
                                            </div>
                                            <div class=" d-flex ">
                                                <p class="font-16 fw-500 d-none">Quantity</p>
                                                <p class="font-16 fw-500">10</p>
                                            </div>
                                            <div class="d-flex ">
                                                <p class="font-16 fw-500 d-none">Sub-total</p>
                                                <p class="font-16 fw-500">5000</p>
                                            </div>


                                        </div>
                                    </div>
                                </div>
                                <div class="cartItem row">
                                    <div class="col">
                                        <div class="itemName d-flex gap-2 ">
                                            <div class="img">
                                                <img src="assets/images/product4.png" alt="">
                                            </div>
                                            <div class="name ">
                                                <p class="font-16 fw-500">Dark blue - Block Dress Dark blue - Block Dress </p>
                                                <p class="font-16 mt-1">Color : Red </p>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <div class="itemDetails d-flex align-items-center justify-content-between">
                                            <div class=" d-flex ">
                                                <p class="font-16 fw-500 d-none">Price</p>
                                                <p class="font-16 fw-500">100</p>
                                            </div>
                                            <div class=" d-flex ">
                                                <p class="font-16 fw-500 d-none">Quantity</p>
                                                <p class="font-16 fw-500">10</p>
                                            </div>
                                            <div class="d-flex ">
                                                <p class="font-16 fw-500 d-none">Sub-total</p>
                                                <p class="font-16 fw-500">5000</p>
                                            </div>


                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="cartItem-summary d-flex gap-3 flex-column mt-4">
                                <div class="row">
                                    <div class="col-8">
                                        <p class="font-16 fw-500">Sub Total (2 items)</p>
                                    </div>
                                    <div class="col-4">
                                        <p class="font-16 fw-600 text-black text-end">150 $</p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-8">
                                        <p class="font-16 fw-500">Discount</p>
                                    </div>
                                    <div class="col-4">
                                        <p class="font-16 fw-600 text-black text-end">150 $</p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-8">
                                        <p class="font-16 fw-500">Shipping Fee</p>
                                    </div>
                                    <div class="col-4">
                                        <p class="font-16 fw-600 text-black text-end">50 $</p>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-8">
                                        <p class="font-16 fw-500">Total</p>
                                    </div>
                                    <div class="col-4">
                                        <p class="font-16 fw-600 text-black text-end">5000 $</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="deliveryInformation d-flex gap-4 mt-5">
                            <div class="customerInfo w-50">
                                <h5>DELIVERY ADDRESS</h5>
                                <h6 class="mt-4">Md Abdul Mannan</h6>
                                <div class="mt-2">
                                    <p>01913-000 520</p>
                                    <p class="mt-2">23/3, Shekertake, Road Number 8,
                                        Mohammadpur, Dhaka 1207.</p>
                                </div>
                            </div>
                            <div class="logisticPatner w-50">
                                <div class="d-flex align-items-center gap-1">
                                    <h5>DELIVERY INFO</h5> <img src="assets/images/icon/van.svg" alt=""
                                                                class="w-auto h-auto">
                                </div>
                                <div class="d-flex  mt-4">
                                    <img src="assets/images/brand/logisticpatner_logo.svg" alt="" class="w-auto h-auto">
                                </div>
                                <div class="d-flex flex-column gap-2 mt-4">
                                    <p class=" fw-500"> <span class="text-black">Order ID :</span> #452140</p>
                                    <p class=" fw-500"><span class="text-black">Payment Method :</span> Cash on delivery
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
        </div>
    </div>
</main>
